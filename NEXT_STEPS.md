...
| Image moderation | Mock / Google | `MockModerationProvider` (keyword substrings) by default. With `AI_PROVIDER=google` + key, `GoogleModerationProvider` uses Vision FACE_DETECTION + OBJECT_LOCALIZATION (person/license-plate) + SAFE_SEARCH, mapped to the shared `decideModeration`. Recognition + moderation share ONE Vision call per photo (`visionClient` memo). Verified live against a portrait (blocked) and an animal (allowed). |
| Card image generation | Mock / crop | Mock: `MockCardGenProvider` -> `mock-card://...` emoji placeholder. Google mode: `CropCardGenProvider` crops the private photo to the Vision subject box (`recognition.subjectBox`), squares + resizes to 1024px JPEG on-device (`expo-image-manipulator`), returns a local `file://` URI — **free, no key, original never leaves the device except as the crop**. Public upload to Supabase Storage + premium AI restyle (Gemini via Edge Function) are later slices. Fallback: crop failure -> `mock-card://` placeholder. |
...
| Auth | Anonymous (Supabase) | When `env.useSupabase` (URL+key set), the app signs in anonymously on startup (`ensureAnonSession`) so each device has a stable `user_id` for RLS. No login screen / named accounts yet; local `currentUserId` stays `'mock-user-001'`. |
| Community sightings | Live (Supabase) when configured | On capture, the PUBLIC-safe version is pushed to `community_sightings` (`src/lib/community.ts`); `MapScreen` fetches other users' rows and merges them in (excluding your own) with a `🌍 N` badge. Private photo + exact GPS never leave the device. Guarded — no-op when Supabase is off. Requires the one-time setup in `supabase/SUPABASE_SETUP.md` (run SQL + enable anonymous sign-in). |
| Map data | Local + community | `MapScreen` reads `useLifeDexStore().sightings` (your local captures) and merges `fetchCommunitySightings()` (other users, public-safe). Reactive to local captures; community fetched once on mount. |
